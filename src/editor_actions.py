...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(f"API unavailable. Creating timeline '{timeline_name}' "
          f"via UI automation.")
    if sys.platform == 'darwin':
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Bolt Optimization: Use a hash map to reduce complexity
            # from O(N*M) to O(N+M). We preserve "first-wins" matching.
            clip_lookup = {}
            for clip in all_clips:
                c_name = clip.GetName()
                if c_name not in clip_lookup:
                    clip_lookup[c_name] = clip

                if hasattr(clip, 'path'):
                    c_base = os.path.basename(clip.path)
                    if c_base not in clip_lookup:
                        clip_lookup[c_base] = clip

            clips_to_add = [clip_lookup[name] for name in clip_names
