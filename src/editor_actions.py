...
def create_new_project(project_name):
    ...
    print(
        "API unavailable. Attempting to create project "
...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(
        "API unavailable. Creating timeline "
        f"'{timeline_name}' via UI automation."
    )
    if sys.platform == "darwin":
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # O(N+M) optimization: Use a hash map for clip lookups
            # We iterate in reverse so the first occurrence in the original
            # list takes precedence in the final dictionary.
            clip_map = {}
            for clip in reversed(all_clips):
                clip_map[clip.GetName()] = clip
                if hasattr(clip, "path") and clip.path:
                    clip_map[os.path.basename(clip.path)] = clip

            clips_to_add = [
                clip_map[name] for name in clip_names if name in clip_map
            ]
