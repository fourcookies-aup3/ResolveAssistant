...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Optimization: Use a hash map for O(1) clip lookup instead of O(M) search
            # This reduces total complexity from O(N*M) to O(N+M)
            clip_map = {}
            for clip in all_clips:
                ...
                # Store by path basename (to match original fallback behavior)
