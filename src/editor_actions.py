...
def create_new_project(project_name):
    ...
    print(f"API unavailable. Creating {project_name} via UI.")
...
def import_media(file_paths):
    ...
    # Fallback to UI Automation
    print("API unavailable. Importing media via UI.")
...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(
        f"API unavailable. Creating {timeline_name} via UI."
    )
    if sys.platform == "darwin":
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Optimization: Use a hash map for O(1) clip lookup.
            # We iterate in reverse to ensure the first occurrence in all_clips
            # remains in the dictionary, preserving 'first-match' behavior.
            clip_lookup = {}
            for clip in reversed(all_clips):
                clip_lookup[clip.GetName()] = clip
                if hasattr(clip, "path") and clip.path:
                    clip_lookup[os.path.basename(clip.path)] = clip

            clips_to_add = []
            for name in clip_names:
                clip = clip_lookup.get(name)
                if clip:
