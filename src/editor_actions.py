...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Build lookup map: Name -> Clip and Basename(path) -> Clip
            # We use reversed order to ensure the first occurrence in all_clips
            # remains the final value in the dictionary, preserving
            # first-match.
            clip_map = {}
            for clip in reversed(all_clips):
                clip_map[clip.GetName()] = clip
                if hasattr(clip, "path") and clip.path:
                    path_basename = os.path.basename(clip.path)
                    clip_map[path_basename] = clip
