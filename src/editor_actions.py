...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Hash map optimization for O(N + M) clip lookup.
            # We iterate in reversed order to preserve 'first-match'
            # behavior: if we overwrite keys, the first occurrence in
            # all_clips (which is processed last) will end up as the
            # final value in the dictionary.
            clip_map = {}
