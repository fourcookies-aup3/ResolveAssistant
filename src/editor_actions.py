...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(
        f"API unavailable. Creating timeline '{timeline_name}' via UI automation."
    )
    if sys.platform == "darwin":
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Optimization: Use a hash map for O(1) lookup (O(N+M) total)
            ...
            for clip in all_clips:
                ...
                if hasattr(clip, "path"):
                    c_basename = os.path.basename(clip.path)
                    clip_lookup[c_basename] = clip
