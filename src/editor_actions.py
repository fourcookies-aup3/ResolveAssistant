...
def create_new_project(project_name):
    ...
    print(
        f"API unavailable. Attempting to create project '{project_name}' "
        "via UI automation."
    )
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # O(N+M) optimization using a hash map for clip lookup.
            # We preserve "first-match" behavior by only storing the first
            # occurrence of a name or basename found in the media pool.
            clip_lookup = {}
            for clip in all_clips:
                ...
                if path:
                    ...
                    if base_name not in clip_lookup:
                        clip_lookup[base_name] = clip

            clips_to_add = [
                clip_lookup[name] for name in clip_names if name in clip_lookup
            ]
