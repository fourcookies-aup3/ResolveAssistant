...
def create_new_project(project_name):
    ...
    print(
        f"API unavailable. Attempting to create project "
        f"'{project_name}' via UI automation."
    )
...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(
        f"API unavailable. Creating timeline '{timeline_name}' "
        f"via UI automation."
    )
    if sys.platform == "darwin":
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Hash map optimization (O(N+M)):
            # We pre-index the clips by name and basename.
            # To preserve first-match behavior, we iterate in reversed order
            # so that earlier clips overwrite subsequent entries and remain as
            # the final matching entry in our index.
            clip_index = {}
            for clip in reversed(all_clips):
                # Always safely query GetName()
                name = clip.GetName()
                if name:
                    clip_index[name] = clip
                # Safely query path attribute if present and non-empty
                if hasattr(clip, "path") and clip.path:
                    basename = os.path.basename(clip.path)
                    if basename:
                        clip_index[basename] = clip

            for name in clip_names:
                if name in clip_index:
                    clips_to_add.append(clip_index[name])
