...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # BOLT OPTIMIZATION: Use a hash map for O(1) clip lookup instead
            # of O(N) linear search. This reduces overall complexity from
            # O(N*M) to O(N+M) where N is pool size and M is targets.
            clip_map = {}
