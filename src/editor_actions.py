...
def create_timeline(timeline_name):
    ...
    # Fallback
    print(
        f"API unavailable. Creating timeline "
...
def add_clips_to_timeline(clip_names):
    if is_api_available():
        ...
        if project:
            ...
            # Optimization: Use a hash map for O(1) lookup instead of O(N)
            # nested loop. This reduces complexity from O(N*M) to O(N+M).
            clip_map = {}
    ...
    for name in clip_names:
        ...
        # 'Append to end of timeline' hotkey
